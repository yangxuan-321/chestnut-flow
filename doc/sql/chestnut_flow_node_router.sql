CREATE TABLE chestnut_flow_node_router(
    ...
    source_node_id bigint DEFAULT 0 NOT NULL,
    target_node_id bigint DEFAULT 0 NOT NULL,
...
CREATE INDEX idx_chestnut_flow_node_router ON chestnut_flow_node_router(flow_id, source_node_id, target_node_id);
...
COMMENT ON COLUMN chestnut_flow_node_router.source_node_id IS '开始节点(流程开始为-1 代表 开始)';
COMMENT ON COLUMN chestnut_flow_node_router.target_node_id IS '结束节点(流程结束为-1 代表 结束)';
...
COMMENT ON COLUMN chestnut_flow_node_router.create_user IS '创建者';
