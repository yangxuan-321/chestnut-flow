...
CREATE UNIQUE INDEX idx_chestnut_flow_workflow ON chestnut_flow_workflow(template_id, flow_version, version);

COMMENT ON TABLE chestnut_flow_workflow IS '流程表';
COMMENT ON COLUMN chestnut_flow_workflow.id IS 'ID';
COMMENT ON COLUMN chestnut_flow_workflow.flow_uuid IS '流程UUID';
...
COMMENT ON COLUMN chestnut_flow_workflow.version IS '流程版本-小版本';
COMMENT ON COLUMN chestnut_flow_workflow.status IS '流程状态';
COMMENT ON COLUMN chestnut_flow_workflow.description IS '流程备注描述';
COMMENT ON COLUMN chestnut_flow_workflow.create_user IS '创建者';
